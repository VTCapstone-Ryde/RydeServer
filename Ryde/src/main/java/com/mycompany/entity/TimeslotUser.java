...
@NamedQueries({
    ...
    @NamedQuery(name = "TimeslotUser.findActiveDriversByTimeslotId", query = "SELECT t.userId FROM TimeslotUser t WHERE t.tsId.id = :tsId AND t.driver = :driver AND t.userId.driverStatus = 1"),
    @NamedQuery(name = "TimeslotUser.findByUserIdAndTimeslotId", query = "SELECT t FROM TimeslotUser t WHERE t.userId.id = :userId AND t.tsId.id = :tsId"),
